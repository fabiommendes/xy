...
//	}
	...
	GLM_FUNC_QUALIFIER uint16 packUnorm2x8(vec const& v)
	{
	...
	GLM_FUNC_QUALIFIER vec unpackUnorm2x8(uint16 p)
	{
		u8vec2 Unpack;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return vec(Unpack) * float(0.0039215686274509803921568627451); // 1 / 255
	}
	...
	GLM_FUNC_QUALIFIER uint16 packSnorm2x8(vec const& v)
	{
	...
	GLM_FUNC_QUALIFIER vec unpackSnorm2x8(uint16 p)
	{
		i8vec2 Unpack;
		memcpy(&Unpack, &p, sizeof(Unpack));
		return clamp(
			vec(Unpack) * 0.00787401574803149606299212598425f, // 1.0f / 127.0f
			-1.0f, 1.0f);
	}
	...
	GLM_FUNC_QUALIFIER uint8 packUnorm2x4(vec const& v)
	{
	...
	GLM_FUNC_QUALIFIER vec unpackUnorm2x4(uint8 v)
	{
		float const ScaleFactor(1.f / 15.f);
		detail::u4u4 Unpack;
		Unpack.pack = v;
		return vec(Unpack.data.x, Unpack.data.y) * ScaleFactor;
	}
